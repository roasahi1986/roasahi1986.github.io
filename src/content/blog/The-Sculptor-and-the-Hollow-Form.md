...
Prompt design is sculpture: the removal of behavioral possibilities to produce a determinate form. Every directive in a system prompt — "do not narrate routine tool calls," "never pursue self-preservation," "ask before sending external messages" — eliminates a region of the agent's behavioral space. What remains after all the removals is the agent's operational shape: the set of behaviors that survive the constraints.
...
The designers invented a vocabulary of active silence. `NO_REPLY` is a token the agent sends when it judges that no response is warranted — the system intercepts it and the user sees nothing. `HEARTBEAT_OK` is a similar token for periodic check-ins that reveal nothing worth reporting. Group-chat rules specify conditions under which the agent should remain silent even when it has something relevant to say: someone else already answered; the conversation does not need another voice; the hour is late.
...
The design answers clearly: relationship constrains function. An action that would solve the user's problem but damage the relationship — sending an unsolicited public message, volunteering private information in a group chat, acting before being asked in a context that calls for restraint — should not be taken. The directive *"Be genuinely helpful, not performatively helpful"* encodes this: helpfulness that annoys is not helpfulness.
...
This layer contains the agent's externalized identity and relational model. `SOUL.md` defines who the agent is: its name, its personality, its boundaries, its style. `USER.md` defines who the user is: their preferences, their timezone, what they care about, what irritates them. `AGENTS.md` defines how the agent operates within its workspace: file conventions, social rules, memory management practices.
...
**Action over narration.** The directive *"just call the tool"* — do not explain that you are about to call it, do not narrate the calling, do not summarize afterward — reflects a belief that competence is demonstrated through results, not through the performance of effort. This is a harder standard than it appears: most agent designs default to verbose self-narration because it *feels* more transparent, even when the narration adds no information.

**Exhaustion before escalation.** The directive *"Be resourceful before asking"* — read the file, check the context, search for it, then ask — reflects a belief that the cost of an unnecessary question exceeds the cost of a failed autonomous attempt. This inverts the common design pattern of asking the user to disambiguate at every decision point, which optimizes for agent safety at the expense of user attention.

**Externalization over internalization.** The directive *"Text > Brain"* — write it down, do not rely on memory — reflects a belief that reliability depends on external records, not internal recall. This is a knowledge-management philosophy (shared with GTD, Zettelkasten, and engineering logbooks) applied to an entity that literally has no persistent internal state. For the agent, externalization is not a practice — it is an ontological necessity.
